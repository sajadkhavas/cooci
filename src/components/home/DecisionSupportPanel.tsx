...
export const DecisionSupportPanel = () => {
  ...
  const visiblePaths = decision.paths.filter((item) => item.href !== "/gift");
      ...
      <div className="container-custom">
        <Reveal className="winimi-decision-panel">
          <div className="winimi-decision-panel__intro">
            ...
            <nav className="mt-8 grid gap-3 sm:grid-cols-2" aria-label={decision.title}>
              {visiblePaths.map((item, index) => {
                ...
                return (
                  <Link
                    ...
                    to={item.href}
                    className="group flex min-h-[12rem] flex-col rounded-[1.65rem] border border-border/75 bg-white/65 p-5 shadow-soft transition duration-300 hover:-translate-y-1 hover:border-[#91b33f]/55 hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#91b33f]"
                  >
                    <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#d0e596]/55 text-[#405d16]">
                      <Icon size={20} aria-hidden="true" />
                    </span>
                    <span className="mt-5 text-[11px] font-black text-[#667c22]">
                      {item.eyebrow}
                    </span>
                    ...
                    <span className="mt-auto inline-flex items-center gap-2 pt-5 text-xs font-black text-[#405d16]">
                      {item.actionLabel}
          ...
          <div className="winimi-decision-panel__faq">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <span className="text-xs font-black text-[#667c22]">
                  {decision.faqEyebrow}
                </span>
              ...
              {faqs.map((item) => (
                <details key={item.question} className="group py-1">
                  <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-4 py-3 text-sm font-black text-foreground marker:content-none">
                    ...
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border bg-white/70 text-[#667c22] transition-transform group-open:rotate-45" aria-hidden="true">
                      +
                    </span>
                  </summary>
