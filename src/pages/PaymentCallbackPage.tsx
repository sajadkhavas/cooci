...
const PaymentCallbackPage = () => {
  ...
  const resolvedOrderId =
    order?.id || (paymentMode === "mock" ? providedOrderId : "");

  const verify = useCallback(async () => {
    if (paymentMode === "mock" && !providedOrderId) {
      setState("unknown");
    ...
    const isConsistentSuccess =
      result.state === "success" &&
      result.order?.paymentStatus === "paid" &&
      Boolean(result.refId);

    setOrder(result.order);
    if (result.state === "success" && !isConsistentSuccess) {
      setError("پاسخ پرداخت برای پاک‌کردن سبد از نظر داخلی کامل نیست.");
      setState("unknown");
      return;
    }
    ...
    <>
      ...
      <section className="bg-gradient-to-b from-secondary/20 to-background section-padding">
        <div className="container-custom max-w-2xl">
          ...
          {state === "loading" && (
            <div
              className="rounded-3xl border border-border bg-card p-10 text-center shadow-card"
              role="status"
            >
              <Loader2
              ...
              <p className="mt-3 leading-8 text-muted-foreground">
                پارامتر بازگشت فقط برای استعلام به سرور ارسال می‌شود و به‌تنهایی اثبات پرداخت نیست.
              </p>
            </div>
          )}
          ...
          {state === "success" && (
            <div className="rounded-3xl border border-emerald-300 bg-card p-7 text-center shadow-card md:p-10">
              ...
              <h1 className="text-2xl font-black md:text-3xl">
                {paymentMode === "backend"
                  ? "پرداخت از سمت سرور تأیید شد"
                  : "پرداخت آزمایشی تأیید شد"}
              </h1>
              <p className="mt-3 leading-8 text-muted-foreground">
                سبد فقط پس از پاسخ سازگار و تأییدشده جریان پرداخت پاک شد.
              </p>
              <div className="my-7 space-y-3 rounded-2xl bg-secondary/60 p-5 text-right">
                {resolvedOrderId && (
                  <div className="flex justify-between gap-3 text-sm">
                    ...
                    <strong dir="ltr">{order?.number || resolvedOrderId}</strong>
                  </div>
                )}
                {order?.refId && (
                  <div className="flex justify-between gap-3 text-sm">
                    <span className="text-muted-foreground">کد پیگیری پرداخت</span>
                    <strong dir="ltr">{order.refId}</strong>
                  </div>
                )}
              ...
              <div className="flex flex-col justify-center gap-3 sm:flex-row">
                {resolvedOrderId && (
                  <Link
                    to={`/account/orders/${encodeURIComponent(resolvedOrderId)}`}
                ...
                <Link
                  to="/products"
                  className="btn-secondary rounded-xl border border-border px-6 py-3"
                >
                  ادامه خرید
                </Link>
              </div>
                ...
                {resolvedOrderId && (
                  ...
                  >
                    <RefreshCcw
                      ...
                      aria-hidden="true"
                    />
                    {retrying ? "در حال ایجاد پرداخت…" : "تلاش مجدد"}
                  </button>
                )}
                <Link
                  to="/cart"
          ...
          {state === "unknown" && (
            <div className="rounded-3xl border border-amber-300 bg-card p-7 text-center shadow-card md:p-10">
              ...
              <h1 className="text-2xl font-black">وضعیت پرداخت مشخص نیست</h1>
              <p className="mt-3 leading-8 text-muted-foreground">
                {error || "هیچ وضعیت پرداختی در مرورگر معتبر فرض نشده است."}
              </p>
              <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
                <button
                  type="button"
