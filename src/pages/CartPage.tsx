            ...
            <div className="grid lg:grid-cols-[1fr_360px] gap-8 items-start">
                    ...
                    <article key={item.id} className="bg-card border border-border rounded-3xl p-4 md:p-5 shadow-soft">
                      <div className="grid md:grid-cols-[120px_1fr_auto] gap-5 items-center">
                        <Link to={`/products/${item.productSlug}`} className="block aspect-square rounded-2xl overflow-hidden bg-secondary">
                          {item.image ? (
                            <img src={item.image} alt={item.imageAlt ?? item.name} className="w-full h-full object-cover" />
                          ) : (
                        ...
                        <div className="space-y-4 md:text-left">
                          ...
                          <div className="inline-flex items-center gap-2 bg-secondary rounded-xl p-1">
                            <button
                              ...
                              className="w-9 h-9 rounded-lg bg-card hover:bg-muted flex items-center justify-center transition-colors"
                          ...
                          <button
                            ...
                            className="text-destructive text-sm font-semibold hover:underline flex items-center gap-1 md:mr-auto"
                          >
                            <Trash2 size={15} />
                            حذف
                          </button>
              ...
              <aside className="bg-card border border-border rounded-3xl p-6 shadow-hover sticky top-28 space-y-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">هزینه ارسال در مرحله بعد و بر اساس شهر و نوع محصول نهایی می‌شود.</p>
                </div>
                ...
                {hasCoolingItems && (
                  <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-sky-900 text-sm leading-7">
                    سبد شما محصول یخچالی دارد؛ ارسال این سفارش فقط برای تهران، کرج و محدوده‌های قابل هماهنگی فعال است.
                  </div>
                )}
