...
const FAQPage = () => {
  ...
  return (
    <>
      <SEO title={shell.metaTitle} description={shell.metaDescription} schema={schema} />
      <section className="bg-secondary/50 py-12"><div className="container-custom text-center"><Breadcrumbs className="mb-4 justify-center" items={[{ name: "خانه", href: "/" }, { name: "سوالات متداول" }]} /><h1 className="heading-1 text-foreground">{shell.heading}</h1><p className="body-large mt-4 text-muted-foreground">{shell.intro}</p></div></section>
      <section className="section-padding"><div className="container-custom max-w-4xl">
        {query.isLoading ? <div className="py-16 text-center" role="status"><Loader2 className="mx-auto mb-3 animate-spin text-primary" aria-hidden="true" />در حال دریافت سوال‌ها…</div> : query.error ? <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-8 text-center text-destructive" role="alert">{query.error instanceof Error ? query.error.message : "دریافت سوال‌ها ناموفق بود."}</div> : (
          <>
            <div className="mb-10 flex flex-wrap justify-center gap-2"><button type="button" onClick={() => setActiveCategory(null)} className={`rounded-full px-4 py-2 text-sm font-medium ${!activeCategory ? "bg-primary text-primary-foreground" : "bg-secondary"}`}>{shell.allLabel}</button>{categories.map((category) => <button key={category} type="button" onClick={() => setActiveCategory(category)} className={`rounded-full px-4 py-2 text-sm font-medium ${activeCategory === category ? "bg-primary text-primary-foreground" : "bg-secondary"}`}>{category}</button>)}</div>
        ...
        <div className="mt-12 rounded-2xl bg-primary/10 p-8 text-center"><h2 className="heading-3 mb-2">{shell.supportTitle}</h2><div className="mt-4 flex flex-col justify-center gap-3 sm:flex-row"><a href={generateWhatsAppUrl(SUPPORT_WHATSAPP_MESSAGE)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl bg-whatsapp px-6 py-3 font-bold text-white"><MessageCircle size={18} aria-hidden="true" />{shell.whatsappLabel}</a><Link to="/contact" className="btn-primary rounded-xl px-6 py-3 font-medium">{shell.contactLabel}</Link></div></div>
      </div></section>
    </>
  );
};
