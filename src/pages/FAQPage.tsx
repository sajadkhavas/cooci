      ...
      <section className="bg-secondary/50 py-12">
        <div className="container-custom text-center">
          <Breadcrumbs
            ...
            items={[{ name: "خانه", href: "/" }, { name: "سوالات متداول" }]}
          />
          <h1 className="heading-1 text-foreground">{shell.heading}</h1>
          <p className="body-large mt-4 text-muted-foreground">{shell.intro}</p>
        </div>
      </section>
      <section className="section-padding">
        <div className="container-custom max-w-4xl">
          {query.isLoading ? (
            ...
            <div
              className="rounded-2xl border border-destructive/30 bg-destructive/5 p-8 text-center text-destructive"
              role="alert"
            >
              {query.error instanceof Error
                ? query.error.message
                : "دریافت سوال‌ها ناموفق بود."}
            </div>
          ) : (
            <>
              <div className="mb-10 flex flex-wrap justify-center gap-2">
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium ${!activeCategory ? "bg-primary text-primary-foreground" : "bg-secondary"}`}
                >
                  {shell.allLabel}
                </button>
                {categories.map((category) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium ${activeCategory === category ? "bg-primary text-primary-foreground" : "bg-secondary"}`}
                  >
                    {category}
                  </button>
                ))}
                ...
                {filtered.map((item) => (
                  ...
                  >
                    <button
                      ...
                      className="flex w-full items-center justify-between p-5 text-right hover:bg-secondary/50"
                    ...
                    {openId === item.id && (
                      <div className="px-5 pb-5">
                        <FaqRichText content={item.answer} />
                      </div>
                    )}
                  </article>
                ))}
              ...
              {filtered.length === 0 && (
                <div className="rounded-2xl border border-border bg-card p-8 text-center">
                  سوالی در این دسته منتشر نشده است.
                </div>
              )}
            </>
          )}
          <div className="mt-12 rounded-2xl bg-primary/10 p-8 text-center">
            <h2 className="heading-3 mb-2">{shell.supportTitle}</h2>
            <div className="mt-4 flex flex-col justify-center gap-3 sm:flex-row">
              ...
              <Link to="/contact" className="btn-primary rounded-xl px-6 py-3 font-medium">
                {shell.contactLabel}
              </Link>
            </div>
          </div>
