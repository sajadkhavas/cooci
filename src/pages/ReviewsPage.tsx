...
import { resolveConditionalContentIndexability } from "@/lib/seo/content-indexability";
...
const ReviewsPage = () => {
  ...
  const reviewQuery = useQuery({
    ...
    queryFn: loadReviewWall,
    initialData: isBackendEnabled ? loaderData?.reviewWall : undefined,
    staleTime: 60_000,
  });

  const reviewWall = reviewQuery.data;
  const reviews = reviewWall?.reviews ?? [];
  const publishedReviewCount = reviewWall?.publishedReviewCount ?? 0;

  const indexability =
    resolveConditionalContentIndexability(publishedReviewCount);
          ...
          {reviews.length > 0 && (
            <div className="mt-6 flex items-center justify-center gap-2">
              ...
              <span className="text-muted-foreground">
                از {publishedReviewCount.toLocaleString("fa-IR")} نظر
              </span>
            </div>
          )}
        </div>
          ...
          {!isBackendEnabled ? (
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {reviews.map((review) => (
                ...
                >
                  ...
                  {review.title && (
                    <h2 className="mb-2 font-black">{review.title}</h2>
                  )}
            ...
            <div className="mx-auto max-w-2xl rounded-3xl border border-border bg-card p-10 text-center">
              ...
              <h2 className="heading-2 mb-3">
                هنوز نظر عمومی تأییدشده‌ای نیست
              </h2>

              <p className="leading-8 text-muted-foreground">
                مشتری پس از تحویل سفارش می‌تواند از صفحه همان سفارش نظر ثبت کند؛
                انتشار پس از بررسی انجام می‌شود.
              </p>
            </div>
          )}
        </div>
