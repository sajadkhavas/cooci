...
const AccountPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setOrdersError(
            error instanceof Error ? error.message : "دریافت سفارش‌ها ناموفق بود.",
          );
        }
      } finally {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "خروج از حساب ناموفق بود.",
      );
      setLoggingOut(false);
    }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "ذخیره پروفایل ناموفق بود.",
      );
    } finally {
    ...
    <>
      ...
      <section className="section-padding bg-gradient-to-b from-secondary/20 to-background">
        <div className="container-custom">
          <header className="mb-8 flex flex-col justify-between gap-5 rounded-3xl border border-border bg-card p-6 shadow-soft md:flex-row md:items-center">
            <div className="flex items-center gap-4 text-right">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground" dir="ltr">
                  {user.mobile || user.email || "شناسه حساب وینیمی"}
                </p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm font-bold hover:border-primary hover:text-primary"
              >
              ...
              <button
                ...
                disabled={loggingOut}
                className="inline-flex items-center gap-2 rounded-xl border border-destructive/30 px-4 py-2.5 text-sm font-bold text-destructive disabled:opacity-50"
              >
                {loggingOut ? (
          ...
          {mode === "mock" && (
            <div
              className="mb-6 flex items-start gap-3 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950"
              role="status"
            >
              <AlertCircle size={19} className="mt-0.5 shrink-0" aria-hidden="true" />
              حساب آزمایشی فقط در محیط توسعه فعال است.
            </div>
          )}
          ...
          {editingProfile && (
            <section
              className="mb-8 rounded-3xl border border-primary/25 bg-card p-6 shadow-soft"
          ...
          <section aria-labelledby="account-orders-title">
            <div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <h2 id="account-orders-title" className="text-2xl font-black">
                سفارش‌های من
              </h2>
              <div className="flex flex-wrap gap-2" aria-label="فیلتر سفارش‌ها">
                {filters.map((item) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-xs font-bold ${filter === item.value ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground shadow-soft"}`}
                  >
                    {item.label}
                  </button>
                ))}
            ...
            {loadingOrders ? (
              <div
                className="rounded-3xl border border-border bg-card p-12 text-center shadow-soft"
                role="status"
              >
                <Loader2
              ...
              <div
                className="rounded-3xl border border-destructive/30 bg-destructive/5 p-10 text-center"
                role="alert"
              >
                <AlertCircle
                  ...
                  aria-hidden="true"
                />
                <h3 className="text-xl font-bold">دریافت سفارش‌ها ناموفق بود</h3>
              ...
              <div className="grid gap-4">
                {filteredOrders.map((order) => (
                  <Link
                    ...
                    className="group rounded-2xl border border-border bg-card p-5 shadow-soft transition hover:border-primary/50"
                  >
                    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <span
                          className={`rounded-full px-3 py-1.5 text-xs font-bold ${statusColor[order.status]}`}
                        >
                          {order.statusLabel || statusLabels[order.status]}
                        </span>
