...
const queryClient = new QueryClient({ defaultOptions: { queries: { staleTime: 5 * 60 * 1000, gcTime: 30 * 60 * 1000, refetchOnWindowFocus: false, refetchOnReconnect: true, retry: 1, networkMode: "offlineFirst" }, mutations: { networkMode: "online", retry: 0 } } });
const lazyElement = (Page: ComponentType): ReactElement => <Suspense fallback={<RouteLoadingFallback />}><Page /></Suspense>;

const App = () => (
  <HelmetProvider><QueryClientProvider client={queryClient}><TooltipProvider><Toaster /><Sonner /><BrowserRouter><AuthProvider><CartProvider><ScrollToTop /><RouteErrorBoundary><Routes><Route element={<SiteLayout />}>
    ...
    <Route path="/checkout" element={<CheckoutGuard>{lazyElement(CheckoutPage)}</CheckoutGuard>} />
    {areDevelopmentMocksEnabled && <Route path="/payment/mock" element={lazyElement(PaymentMockPage)} />}
    <Route path="/payment/callback" element={lazyElement(PaymentCallbackPage)} />
    <Route path="/account/login" element={lazyElement(LoginPage)} />
    <Route path="/account" element={<ProtectedRoute>{lazyElement(AccountPage)}</ProtectedRoute>} />
    <Route path="/account/orders/:orderId" element={<ProtectedRoute>{lazyElement(OrderDetailPage)}</ProtectedRoute>} />
    <Route path="*" element={lazyElement(NotFoundPage)} />
  </Route></Routes></RouteErrorBoundary></CartProvider></AuthProvider></BrowserRouter></TooltipProvider></QueryClientProvider></HelmetProvider>
);
export default App;
